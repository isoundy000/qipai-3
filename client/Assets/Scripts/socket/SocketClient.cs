...
public static class SocketClient
{
    ...
    private static Dictionary<string, Action<string>> handlers = new Dictionary<string, Action<string>>();  //路由处理函数
    private static List<SocketMsg> msgCache = new List<SocketMsg>();                                  //缓存的消息列表
    private static object lockObj = new object();

    private static string socketOpenCmd = "_open_";
    private static string socketCloseCmd = "_close_";
    ...
    {
        handlers[socketCloseCmd] = handler;
    }
    ...
    {
        handlers.Remove(socketCloseCmd);
    }
    ...
    {
        handlers[socketOpenCmd] = handler;
    }
        ...
        private void Recive()
        {
            try
            {
                ...
                asyncResult =>
                {
                    ...
                    try
                    {
                        length = mySocket.EndReceive(asyncResult);
                    }
                    catch (Exception e)
                    {
                        Debug.Log(e);
        ...
        private void HandleMsg(List<byte> tmpBytes)
        {
            ...
            try
            {
                ...
                if (tmpBytes[0] == 1)   // 自定义消息
                {
                    ...
                    msg.cmd = route[(tmpBytes[1] << 8) | tmpBytes[2]];
